...
크레인 행동 (x_t^crane):
  PICKING(k, src_stack, run_id)        — WIP k를 스택에서 설비로 올림
  STORE(k, dst_stack, run_id)       — 출력재 k를 설비에서 스택으로 내림
...
생산 행동 (x_t^prod):
  START_PROCESS(run_id) — 현재 K_mach로 run 시작
  CONTINUE — 기존 생산 유지
  NONE — 생산 관련 이벤트 없음

PRE_POSITION vs RESTORE 차이:
  RESTORE      : 버퍼 → 아무 야드 스택 (버퍼 공간 확보가 주목적)
  PRE_POSITION : 버퍼 → 전략적 스택 (다음 run LOAD를 빠르게 하는 것이 주목적)
...
PROD_DIRECT_START = "DIRECT_START"   # 원자재 run: 야드에 적재 X. 바로 생산 시작한다고 가정
...
class CraneAction:
    ...
    run_id:     Optional[int] = None   # 연결 run (PICKING, STORE)
    ...
    def __repr__(self):
        ...
        if self.run_id   is not None: args.append(f"run={self.run_id}")
...
class ProdAction:
    ...
    def __repr__(self):
        if self.run_id is not None:
            return f"{self.type}(run={self.run_id})"
        return self.type

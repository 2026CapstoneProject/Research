...
Rolling Horizon DLA 정책:
  ...
  - generic run / unique output WIP이 추가된 DIDPPy 모델 사용
...
def rolling_horizon_policy(
    ...
    if not is_available() or len(state.Q_rem) == 0:
        action = greedy_policy(state, wip_data, run_data)
        return action, float("inf")

    # ── DIDPPy 모델 빌드 ──────────────────────────────────────
    active_run_ids = sorted(state.Q_rem)
    # extract_first_action()가 model_builder와 동일한 WIP 인덱스 순서를
    # 사용해야 LOAD_{wi}_{ri} → 실제 WIP ID 매핑이 어긋나지 않는다.
    active_wip_ids = compute_relevant_wip_ids(state, run_data, active_run_ids)

    model = build_didp_model(
        state, wip_data, run_data, machine_times, horizon
    )
    ...
    # ── 첫 번째 전이 → Action 변환 ────────────────────────────
    active_run_ids_list  = sorted(state.Q_rem)
    active_wip_ids_list  = active_wip_ids

    action = extract_first_action(
        result.transitions,
        state, wip_data, run_data,
        active_wip_ids_list, active_run_ids_list,
    )
    ...
    greedy_action = greedy_policy(state, wip_data, run_data)
